...
def admin_profile():
    ...
    if request.values.get('add_new_event'):
        ...
        add_new_event(conn, request.values.get('new_event_name'), request.values.get('theme'),
                      ...
                      int(get_location_id(conn, venue_id, location_name)) , request.values.get('new_event_description'), request.values.get('status'))
    ...
    #     detailed_info = False
        ...
        update_event(conn, session['event_id'], request.values.get('edit_event_name'), request.values.get('edit_theme'),
                      ...
                      request.values.get('edit_event_beginning_dat')[6:11] + "-" + request.values.get('edit_event_beginning_dat')[3:5]+ "-" + request.values.get('edit_event_beginning_dat')[0:2],
                      request.values.get('edit_event_expiration_dat')[6:11] + "-" + request.values.get('edit_event_expiration_dat')[3:5] + "-" + request.values.get('edit_event_expiration_dat')[0:2],
                      request.values.get('edit_event_start_time'), request.values.get('edit_event_end_time'), request.values.get('edit_organizer'),
                      int(get_location_id(conn, venue_id, location_name)) , request.values.get('edit_event_description'), request.values.get('edit_status'))
